...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, x1, layers, dt, lb, ub, q):
        ...
        input  lb: lower boundary [xl,tl]
               ...
               x0: choice of x at t = 0 ???
               u0: u(x0, t=0)
               x1: [[lb],[ub]].T  use this to calculate boundary condition
               layer: neural network layer [1, 200, 200, 200, 200, q+1]
               dt: change of time, dt = 0.8
               q: number of stage (>= 1)
        ...
        # Load IRK weights
        ## IRK_weights: first (q, q) is a_ij, last (1, q) is b_j ?
        ## IRK_times: c_j
        ...
        self.IRK_times = tmp[q**2+q:] #shape = (q,1)
        ...
        ## why self.q, and what is this???
        self.dummy_x0_tf = tf.placeholder(tf.float32, shape=(None, self.q)) # dummy variable for fwd_gradients (forward gradient)
    ...
    def neural_net(self, X, weights, biases):
        ...
        for l in range(0,num_layers-2):
            W = weights[l]
            b = biases[l]
            H = tf.tanh(tf.add(tf.matmul(H, W), b)) # hidden layer
        W = weights[-1]
        b = biases[-1]
        Y = tf.add(tf.matmul(H, W), b) # final layer
        return Y
    ...
    def net_U0(self, x):
        ...
        U0 = U1 - self.dt*tf.matmul(F, self.IRK_weights.T) # important!
        return U0

    def net_U1(self, x):
        U1 = self.neural_net(x, self.weights, self.biases) # same as net_U0
...
if __name__ == "__main__": 
    ...
    # Initial data
    noise_u0 = 0.0 # use this to change noise
