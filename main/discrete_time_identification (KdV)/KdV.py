...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, x1, u1, layers, dt, lb, ub, q):
        """
        input  lb: lower boundary 
               ub: upper boundary 
               x0: use x0 to predict u(n)_q
               u0: u(n)
               x1: use x1 to predict u(n+1)_q
               u1: u(n+1)
               layer: neural network layer [1, 50, 50, 50, 50, q]
               dt: change of time, dt = 0.6, which is (0.8-0.2)
               q: number of stage (>= 1)
        ...
        self.IRK_alpha = weights[0:-1,:] # (q, q)
        self.IRK_beta = weights[-1:,:] # (1, q)       
